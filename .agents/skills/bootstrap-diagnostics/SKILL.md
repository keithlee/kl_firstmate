...
- `NO_MISTAKES_IDENTITY: <detail>` - the configured no-mistakes pin could not be resolved or its path/build identity did not match. Keep ship dispatch stopped, inspect the private `config/no-mistakes` record and the executable it names, repair all required `path`, `realpath`, and `identity` fields only with the captain's approval, then rerun bootstrap and confirm the resolved executable before dispatch.
