...
if [ "$KIND" = ship ] && [ -n "$CREW_BRANCH" ] && [ -n "${FM_NO_MISTAKES_BIN:-}" ]; then
  ...
  if [ "$RUN_SOURCE" = coarse ]; then
    ...
    if [ -n "$outcome" ]; then
      case "$outcome" in
        ...
        checks-passed)
          if fm_nm_handback_readiness "$WT" "$NM_TIMEOUT" "$RUN_OUT"; then
            RUN_STATE="done"; RUN_DETAIL="checks green: PR ready for review"
          else
            RUN_STATE="blocked"; RUN_DETAIL="feedback-blocked: live handback readiness failed or is unreadable"
          fi
          ;;
        failed)        RUN_STATE=failed; RUN_DETAIL="run failed" ;;
        cancelled)     RUN_STATE=failed; RUN_DETAIL="run cancelled" ;;
